...
public class ASMUtils implements Opcodes {
	...
	public static Type[] castTypes(Type[] argumentTypes) {
		int size = argumentTypes == null ? 0 : argumentTypes.length;
		if (size > 0) {
	...
	public static int getConstInsnByType(Type type) {
		int op = -1;
		switch (type.getSort()) {
		case Type.BOOLEAN:
		case Type.BYTE:
	...
	public static InsnNode newConstInsnNodeByType(Type type) {
		int op = getConstInsnByType(type);
		if(op != -1) {
